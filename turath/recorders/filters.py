...
def run(playwright: Playwright) -> None:
    ...
    context = browser.new_context(
        **device,
        record_video_dir="./clips/filters",
        record_video_size={
    ...
    click_with_ripple(page, page.get_by_role("button", name="بحث"))
    add_title(page, "خواص تصفية البحث", mainTitle=True)
    click_with_ripple(page, page.get_by_role("textbox", name="كلمات البحث"))
    page.get_by_role("textbox", name="كلمات البحث").fill("الصحابة")
    add_title(page, "١. التصفية حسب الكتاب")
    click_with_ripple(page, page.get_by_role("button", name="تصفية البحث"))
    click_with_ripple(page, page.get_by_role("button", name="الكتاب"))
    click_with_ripple(page, page.get_by_role("searchbox", name="كلمات البحث"))
    page.get_by_role("searchbox", name="كلمات البحث").fill("رجال حول")
    page.get_by_role("checkbox").check()
    click_with_ripple(page, page.get_by_role(
        "button", name="عرض خيارات التصفية"))
    click_with_ripple(page, page.get_by_role("button", name="إزالة التصفية"))

    pixel_ratio = page.evaluate("() => window.devicePixelRatio")
    print("devicePixelRatio:", pixel_ratio)

    add_title(page, "٢. التصفية حسب المؤلف")
    click_with_ripple(page, page.get_by_role("button", name="تصفية البحث"))
    click_with_ripple(page, page.get_by_role("button", name="المؤلف"))
    click_with_ripple(page, page.get_by_role(
        "button", name="زكريا الأنصاري").get_by_role("checkbox"))
    click_with_ripple(page, page.get_by_role(
        "button", name="عرض خيارات التصفية"))
    click_with_ripple(page, page.get_by_role("listitem").filter(
        has_text="المؤلف (١)").get_by_label("إزالة التصفية"))

    add_title(page, "٣. التصفية حسب القسم")
    click_with_ripple(page, page.get_by_role("button", name="تصفية البحث"))
    click_with_ripple(page, page.get_by_role("button", name="القسم"))
    click_with_ripple(page, page.get_by_role(
        "button", name="العقيدة").get_by_role("checkbox"))
    click_with_ripple(page, page.get_by_role(
        "button", name="عرض خيارات التصفية"))

    add_title(page, "٤. التصفية حسب تاريخ الوفاة")
    click_with_ripple(page, page.get_by_role("button", name="تصفية البحث"))
    click_with_ripple(page, page.get_by_role("button", name="تاريخ الوفاة"))
    click_with_ripple(page, page.get_by_role("textbox", name="البدأ"))
    page.get_by_role("textbox", name="البدأ").fill("٣٢2")
    click_with_ripple(page, page.get_by_role("textbox", name="النهاية"))
    page.get_by_role("textbox", name="النهاية").fill("١٣٠0")
